...
namespace RTSEngine.NPC.BuildingExtension
{
    public class NPCBuildingCreator : NPCComponentBase, INPCBuildingCreator
    {
        ...
        private bool OnCreateBuildingRequestInternal(NPCBuildingRegulator instance, IBuilding buildingCenter = null)
        {
            ...
            IReadOnlyList<string> builderCodes = npcBuildingConstructor.GetBuildingBuilderCodes(instance.Prefab.Code);
            if (!builderCodes.IsValid())
            {
            ...
            for (int i = 0; i < builderCodes.Count; i++)
            {
                NPCUnitRegulator nextBuilderRegulator;
                if ((nextBuilderRegulator = npcUnitCreator.GetActiveUnitRegulator(builderCodes[i])).IsValid())
                {
                    IBuilder nextBuilder = null;
                    int j = 0;
                    while (j < nextBuilderRegulator.Instances.Count)
                    {
                        if (nextBuilderRegulator.Instances[j].BuilderComponent.IsValid())
                        {
                            nextBuilder = nextBuilderRegulator.Instances[j].BuilderComponent;
                            break;
                        }
                        j++;
                    }
                    ...
                    if (nextBuilder.IsValid())
                        for (int t = 0; t < nextBuilder.CreationTasks.Count; t++)
                        {
                            if (nextBuilder.CreationTasks[t].Prefab.Code == instance.Prefab.Code)
                            {
                                nextBuildingCreationTask = nextBuilder.CreationTasks[t];
                                break;
                            }
                        }
                }
            ...
            activeBuildingRegulatorLogs = buildingCenterRegulators.Values
                ...
                { 
                    ...
                    .Select(regulator => new NPCActiveFactionEntityRegulatorLogData(
                        ...
                        creators: npcBuildingConstructor.GetBuildingBuilderCodes(regulator.instance.Prefab.Code)?.ToArray()))
                    .ToArray()
                })
                .ToList();
        }
